...
const ProjectOverview = () => {
  ...
  const fetchOverview = async () => {
    try {
      const res = await axiosClient.get(
        `/tenant/projects/${projectId}/overview`,
      );
      setData(res.data);
    } catch (err) {
  ...
  if (!data)
    return <div className="text-white/50">Failed to load project data.</div>;
    ...
    <div className="max-w-5xl w-full flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="group relative bg-[#19191a] border border-white/[0.06] hover:border-white/[0.12] rounded-xl p-4 flex flex-col gap-3 overflow-hidden transition-all duration-300">
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-400/50 mt-1 font-mono">
              {allowedPercent}% of total
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#19191a] border border-white/[0.06] rounded-xl p-4 flex flex-col">
          ...
          <div className="flex flex-col gap-1">
            {topBlockedRules.length === 0 ? (
              <p className="text-[12px] text-white/25 py-4 text-center font-mono">
                No blocks recorded today
              </p>
            ) : (
            ...
            {recentActivity.length === 0 ? (
              ...
              recentActivity.map((log: any, i: number) => (
                ...
                >
                  ...
                  <span
                    className={`text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded-md border font-mono ${
                      log.allowed
      ...
      <div className="bg-[#19191a] border border-white/[0.06] rounded-xl overflow-hidden flex flex-col">
        <div className="px-4 py-3.5 border-b border-white/[0.05] flex items-center justify-between">
          <div>
            ...
            <h3 className="text-[13px] font-semibold text-white/85 tracking-wide leading-none">
              Rules
              <span className="ml-2 text-[10px] text-white/25 font-mono font-normal">
                ({rules.length})
              </span>
            </h3>
          </div>
        ...
        <div className="flex flex-col gap-2 p-3">
          {rules.length === 0 ? (
            <p className="text-[12px] text-white/25 py-8 text-center font-mono">
            ...
            rules.map((rule: any) => (
              ...
              >
                ...
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  ...
                  <span className="text-[13px] font-semibold text-white/85 font-mono tracking-wide truncate">
                    {rule.name}
                  </span>
                </div>
