...
const TopBar = () => {
    ...
    const projectName = urlName ? urlName : (isProjectRoute ? "Project Settings" : null);
    ...
    return (
        <header className="flex items-center justify-between h-[52px] min-h-[52px] px-4 bg-dashboard-primary border-b border-white/10">
            ...
            <div className="flex items-center gap-2 lg:gap-3">
                ...
                <Link href="/dashboard" className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-white/5 transition-colors duration-200 group">
                    ...
                    <span className={`text-sm font-medium transition-colors ${projectName ? "text-white/60 group-hover:text-white" : "text-white/90 group-hover:text-white"}`}>
                        Yash's Workspace
                    </span>
                    {!projectName && (
            ...
            <div className="flex items-center gap-4">
                {/* Search Bar */}
                <button className="flex items-center gap-2 h-8 px-3 rounded-lg bg-dashboard-primary border border-white/10 text-white/50  hover:border-dashboard-border transition-all duration-200 min-w-[200px] hover:cursor-pointer">
                    <Search size={14} />
                    <span className="text-xs flex-1 text-left">Search...</span>
                    <kbd className="flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-dashboard-primary border border-white/10 text-[10px] text-dashboard-muted-foreground font-mono">
                        ⌘K
                    </kbd>
                </button>
