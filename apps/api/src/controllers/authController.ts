...
export default async function authController(req: Request, res: Response) {
    try{
        ...
        if(existingUser){
            ...
            const token = jwt.sign({tenantId:existingUser.id,email:existingUser.email},process.env.JWT_SECRET as string);
        ...
        if(user){
            ...
            const token = jwt.sign({tenantId:user.id,email:user.email},process.env.JWT_SECRET as string);
            console.log("token length creating", token.length);
            if(token){
